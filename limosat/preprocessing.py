...
gdal.SetConfigOption('GDAL_VRT_RAWRASTERBAND_ALLOWED_SOURCE', os.environ['GDAL_VRT_RAWRASTERBAND_ALLOWED_SOURCE'])
gdal.SetConfigOption('VRT_ALLOW_MEM_DRIVER', os.environ['VRT_ALLOW_MEM_DRIVER'])
...
def get_n(filename, pols=('HH', 'HV'), factor_hh=2, factor_hv=5):
    ...
    n = Nansat(filename, mapper='sentinel1_l1')
    bb = []
    for pol in pols:
...
def _warmup_nansat(path):
    try:
        Nansat(path, mapper='sentinel1_l1')
    except Exception as e:
