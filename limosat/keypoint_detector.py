...
class KeypointDetector:
    ...
    def compute_descriptors(self, keypoints_with_tags, img, polarisation='s0_HV', normalize=True):
        ...
        if polarisation == 'dual_polarisation':
            ...
            if descriptors_hh is None or len(descriptors_hh) == 0 or not keypoints_output_hh:
                return None, None, None
            ...
            if descriptors is None or len(descriptors) == 0 or not keypoints_output:
                return None, None, None
    ...
    def detect_keypoints(
        ...
        """
        Unified keypoint detection in windows, with optional overlap and filtering.
        """
        if stride is None:
            stride = window_size
        ...
        if check_existing and points is not None and not points.empty:
            points_poly_kp = np.array([(geom.x, geom.y) for geom in points.geometry])
            cols, rows = img.transform_points(
                points_poly_kp[:, 0], points_poly_kp[:, 1], DstToSrc=1, dst_srs=NSR(3413)
            )
            existing_keypoints_coords = np.vstack((cols.flatten(), rows.flatten())).T

        windows = view_as_windows(img0, window_shape=(window_size, window_size), step=stride)
        n_windows_row, n_windows_col, _, _ = windows.shape

        keypoints = []
        for i in range(n_windows_row):
            for j in range(n_windows_col):
                x_start, y_start = j * stride, i * stride
                
                if check_existing:
                    x_end, y_end = x_start + window_size, y_start + window_size
                    num_existing_kp = np.sum(
                        (existing_keypoints_coords[:, 0] >= x_start) & (existing_keypoints_coords[:, 0] < x_end) &
                        (existing_keypoints_coords[:, 1] >= y_start) & (existing_keypoints_coords[:, 1] < y_end)
                    )
                    if num_existing_kp >= 1:
                        continue

                window = windows[i, j]
                kps_in_window = self.model.detect(window, None)
                strong_kps = [kp for kp in kps_in_window if kp.response > response_threshold]
                
                sorted_kps = sorted(strong_kps, key=lambda kp: kp.response, reverse=True)
                top_kps = sorted_kps[:top_n]

                for kp in top_kps:
                    kp.pt = (kp.pt[0] + x_start, kp.pt[1] + y_start)
                    kp.angle = img.angle
                    kp.octave = octave
                    keypoints.append((kp, None))

        img_h, img_w = img0.shape
        filtered_keypoints_with_tags = [
            item for item in keypoints
            if (border_size <= item[0].pt[0] <= img_w - border_size and
                border_size <= item[0].pt[1] <= img_h - border_size)
        ]
        ...
        return self.compute_descriptors(
            filtered_keypoints_with_tags, img, polarisation=1, normalize=False
        )
    ...
    def get_pixel_coords(self, nansat_obj, geom_x, geom_y):
        ...
        if cols.size > 0 and np.isfinite(cols[0]) and np.isfinite(rows[0]):
            return int(round(cols[0])), int(round(rows[0]))
    ...
    def keypoint_from_point(
        ...
        for original_idx, point_row in points_gdf_for_current_image.iterrows():
            ...
            try:
                ...
                if not ((r1 - r0) >= window_size_for_detection * 0.8 and (c1 - c0) >= window_size_for_detection * 0.8):
                    continue 
                ...
                if best_kp_in_patch is not None:
                    ...
                    final_kp_to_add = cv2.KeyPoint(
                        x=float(kp_final_c), y=float(kp_final_r), size=float(best_kp_in_patch.size),
                        octave=int(octave), angle=float(img.angle), response=float(best_kp_in_patch.response)
                    )
